...
skel_plik = '10cv_no' # Please provide n-fold training core filenames if use_classic_approach is True (filenames MUST contain '_no' part), copy files to ./10cv_FS
skel_plik1 = 't-10cv_no' # Please provide n-fold testing core filenames if use_classic_approach is True (filenames MUST contain '_no' part), copy files to ./10cv_FS
fs_data = '5HT_padel_out_2D_names.csv' # Please provide full filename if perform_FS is True
...
if perform_FS is True:
    ...
    train_inds, test_inds = next(GroupShuffleSplit(n_splits=1, train_size=0.7, test_size=0.3).split(X, groups=groups))
    ...
    train_set.to_csv(file_name_train, index=False, sep="\t")
    test_set.to_csv(file_name_test, index = False, sep="\t")
    ...
    if save_pojo_or_mojo is True:
        if aml.leader.have_pojo is True:
            aml.leader.download_pojo(get_genmodel_jar = True)
        if aml.leader.have_mojo is True:
            aml.leader.download_mojo(get_genmodel_jar = True)
...
if use_classic_approach is True and perform_FS is False:
    ...
    # save h2o model
    print('Saving leader h2o model: ')
    h2o.save_model(aml_10cv.leader)
    # Download POJO or MOJO
    if save_pojo_or_mojo is True:
        if aml_10cv.leader.have_pojo is True:
            aml_10cv.leader.download_pojo(get_genmodel_jar = True)
        if aml_10cv.leader.have_mojo is True:
            aml_10cv.leader.download_mojo(get_genmodel_jar = True)
    ...
    if ("StackedEnsemble" in aml_10cv.leader.key) == True:
        # get the metalearner name
        se_meta_model = h2o.get_model(m.metalearner()['name'])
        h2o.save_model(se_meta_model)
        ...
        for model in m.params['base_models']['actual']:
            model_list.append(model['name'])
            h2o.save_model(h2o.get_model(str(model['name'])))
...
if use_classic_approach is False and perform_FS is True:
        ...
        # save h2o model as pojo or mojo
        print('Saving leader h2o model: ')
        h2o.save_model(aml_10cv.leader)
        # Download POJO or MOJO
        if save_pojo_or_mojo is True:
        ...
        if ("StackedEnsemble" in aml_10cv.leader.key) == True:
            ...
            h2o.save_model(se_meta_model)
            h2o_cv_data = se_meta_model.cross_validation_holdout_predictions()
            pred_obs = h2o_cv_data.cbind([training_frame[training_frame.col_names[len(training_frame.col_names)-2]],
                                        training_frame['Fold_no']])
